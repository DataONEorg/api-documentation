...
class FunctionLoader(docLoader):
    ...
    def _functionToPy(self, mname, fname):
        ppart = {'ssl': '',
        ...
        for param in func['params']:
            pnames.append(signatureParam(param['name']))
        res.append("def %s(%s):" % (fname, ",".join(pnames)))
        #"""
        res.append('  """')
        #description
        ...
        if len(func["params"]) > 0:
            ...
            for param in func["params"]:
                pname = cleanParam(param['name'])
                if pname == "session":
                    pdescr = "|session|"
                else:
                    pdescr = param['descr']
                ptype = self._toTypeString(param['type'])
                xmittype = param['xmit'].lower()
                if pname == "object":
                    pass
                else:
                    pdescr += " " + ppart[xmittype]
                res.append(f"    {pname} ({ptype}): {pdescr}")

        # Returns
        res.append("")
        res.append("  Returns:")
        rtype = self._toTypeString(func['rtype'])
        rdescr = func['rdescr']
        res.append(f"    {rtype}: {rdescr}")
        ...
        for exc in func['exceptions']:
            ...
            dcode = exc['detailCode']
            ename = exc['name']
            edescr = exc['descr']
            res.append(f"    Exceptions.{ename}: {edescr} (errorCode={ecode}, detailCode={dcode})")
    ...
    def _functionToText(self, mname, fname):
        ...
        for param in func['params']:
            ...
            if param['name'] == "object":
                res.append(self.formatBlock("%s(%s): %s\n" % (self._cleanParamName(param['name']), param['descr']), param['type'], "    ", "    "))
            else:
                res.append(self.formatBlock("%s: %s %s\n" % (self._cleanParamName(param['name']), param['descr'], ppart[xmittype]), "     ", "     "))
        ...
        for exc in func['exceptions']:
            ...
            res.append(
                "   :raises Exceptions.%s: ``(errorCode=%s, detailCode=%s)``" % (exc['name'], ecode, exc['detailCode']))
    ...
    def generatePython(self):
        res = {}
        for module in self.data['order']:
            mdata = ['import Exceptions', 'import Types', '', '']
            for fname in self.data['modules'][module]['order']:
                mdata += self._functionToPy(module, fname)
            res[module] = mdata
        return res
...
def generateDocs(fname, destpath):
    ...
    text = functions.generatePython()
    for k in list(text.keys()):
        logging.info('Generating module %s' % k)
        fname = os.path.join(destpath, "%s.py" % k)
